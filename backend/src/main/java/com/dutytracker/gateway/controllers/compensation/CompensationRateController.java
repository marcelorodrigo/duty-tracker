...
public class CompensationRateController {
    ...
    @GetMapping
    @Operation(summary = "Get compensation rate table", description = "Retrieve compensation rates, optionally filtered by employee type")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Compensation rates retrieved successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = CompensationRateTableResponse.class)))
    })
    ...
    @PostMapping
    @Operation(summary = "Create compensation rate", description = "Create a new compensation rate with percentage and label")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Compensation rate created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = CompensationRateResponse.class))),
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update compensation rate", description = "Update the percentage and label of a compensation rate")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Compensation rate updated successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = CompensationRateResponse.class))),
            @ApiResponse(responseCode = "400", description = "Invalid rate data"),
