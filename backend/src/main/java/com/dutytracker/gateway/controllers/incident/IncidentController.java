...
public class IncidentController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Incident created successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = IncidentResponse.class))),
    ...
    @GetMapping
    @Operation(summary = "List incidents", description = "Retrieve all incidents, optionally filtered by on-call period")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Incidents retrieved successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = IncidentListResponse.class)))
    })
    public ResponseEntity<IncidentListResponse> list(
            @Parameter(description = "On-call period ID to filter incidents (optional)")
    ...
    @Operation(summary = "Calculate overtime entries", description = "Generate overtime entries for a specific incident")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Overtime entries calculated successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = OvertimeEntriesResponse.class))),
            @ApiResponse(responseCode = "404", description = "Incident not found")
    })
