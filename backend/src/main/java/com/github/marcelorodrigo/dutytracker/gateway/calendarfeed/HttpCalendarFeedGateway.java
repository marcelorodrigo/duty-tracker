 ...
 *
 * <p>Two layers guard the outbound connection:
 *
 * <ol>
 *   <li>{@link CalendarFeedUrlValidator} validates the URL string (scheme, host, literal private IPs).
 *   <li>{@link CalendarFeedResolverProvider} installs a DNS resolver that refuses to resolve the
 *       calendar feed host to anything other than a globally routable unicast address.
 * </ol>
 *
 * <p>The request is made using the original hostname, so the JDK client derives the {@code Host}
 * header, TLS SNI and certificate validation from that hostname while the resolver guarantees the
 * resolved IP is public. Validation runs on every resolution, removing the time-of-check/time-of-use
 * window between URL validation and the connection.
 */
@Component
